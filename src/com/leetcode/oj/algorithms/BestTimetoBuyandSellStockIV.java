...
public class BestTimetoBuyandSellStockIV {
	...
	int nProfit=0,nDeficit=0;
	ArrayList<Integer> profit= new ArrayList<Integer>();
    ArrayList<Integer> defecit= new ArrayList<Integer>();
	...
	public int maxProfit(int k, int[] prices) {
        ...
        for(int i=1;i<prices.length-1;i++){
            if(trend==1&&prices[i]>prices[i+1]){
                currentMax=prices[i];
                profit.add(currentMax-currentMin);
                nProfit++;
                System.out.print("profile:");
                System.out.println(currentMax-currentMin);
            }else if(trend==-1&&prices[i]<prices[i+1]){
                currentMin=prices[i];
                profit.add(currentMin-currentMax);
                nDeficit++;
                System.out.print("profile:");
        ...
        if(trend==1){
            ...
            currentMin=prices[prices.length-1];
            profit.add(currentMin-currentMax);
            nDeficit++;
            System.out.print("profile:");
            System.out.println(currentMin-currentMax);
        ...
        while(nProfit>k){
            ...
            if(profit.get(profit.size()-1)<0){
            	profit.remove(profit.size()-1);
            	nDeficit--;
            }
        ...
        for(Integer i:profit){
        	if(i>0){
        		sumOfProfit=sumOfProfit+i.intValue();
        	}
        }

        return (sumOfProfit>0?sumOfProfit:0);
    }
	
	public void reduceTransaction(){
		int index=getIndexOfSmallestABS();
		
		if(profit.get(index)>0&&(index==0||index==profit.size())){
			nProfit--;
			profit.remove(index);
			return;
		}
		...
		if(index==0){
			System.out.println("lower:"+String.valueOf(profit.get(index)));
			profit.set(index,profit.get(index)+profit.get(index+1));
			profit.remove(index+1);
			
		}else if(index==profit.size()-1){
			System.out.println("upper:"+String.valueOf(profit.get(index)));
			profit.set(index,profit.get(index-1)+profit.get(index));
			profit.remove(index-1);
		}else{
			System.out.println("middle:"+String.valueOf(profit.get(index)));
			profit.set(index,profit.get(index-1)+profit.get(index)+profit.get(index+1));
			profit.remove(index+1);
			profit.remove(index-1);
		}
	...
	public int getIndexOfSmallestABS(){
		int index=0;
		int value=Math.abs(profit.get(0));
		for (int i=0;i<profit.size();i++){
			if(value>Math.abs(profit.get(i))){
				value=Math.abs(profit.get(i));
				index=i;
			}
		}
		System.out.println("Minimum ABS Value:"+String.valueOf(value));
	...
	public static void main(String[] args) {
		...
		int maxprofit=stk.maxProfit(k, prices);
